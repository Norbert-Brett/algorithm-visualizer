...
export default function InsertionSortVisualization({
  ...
  const insertionSort = async () => {
    ...
    for (let i = 1; i < n; i++) {
      // Mark current element as being inserted
      arr[i].isInserting = true;
      setArray([...arr]);
      ...
      while (j >= 0) {
        ...
        if (arr[j].value > key) {
          // Shift element to the right with new unique ID
          arr[j + 1] = { 
      ...
      // Insert the key at correct position with new unique ID
      arr[j + 1] = {
        id: Date.now() * 1000 + Math.random() * 1000,
        value: key,
        isSorted: true,
            ...
            {array.map((item, index) => (
              ...
              >
                <motion.div
                  ...
                  animate={{
                    scale: item.isInserting || item.isComparing ? 1.1 : 1,
                    y: item.isInserting ? -10 : 0,
                  }}
